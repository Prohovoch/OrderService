...
namespace OrderService.src.Deal.Admin
{
    public class LookCompleteOrders(ApplicationDbContext dbContext) : Endpoint<LookCompleteOrdersRequest, GetOrdersResponse>
    {
        ...
        public override async Task HandleAsync(LookCompleteOrdersRequest req, CancellationToken ct)
        {
            ...
            var completeOrders = await _dbContext.Orders
                .Where(o => o.Status == OrderStatus.Closed).AsNoTracking().Select(x => new LookCompleteOrdersResponse
                {
                    ...
                    CompletedAt = (DateTimeOffset)x.CompletedAt!, // XDDDDD or x.CompletedAt!.Value
    ...
    public class LookCompleteOrdersValidator : Validator<LookCompleteOrdersRequest>
    {
